...
$(function(){
	...
	var $reduce_cost = $page.find("[name='reduce_cost']");
	
	var $title = $page.find("[name='title']");
	var $least_cost = $page.find("[name='least_cost']");
...
// 
//			}  
			,reduce_cost : function(){
				
				var v = $reduce_cost.val() || 0;
				 
		 		if(+v <= 0){

			 		$("#reduce_costErrorMsg").css({
			 				'display' : 'block'
			 		})
	...
	$least_cost.blur(function(){
		checks['least_cost']();
	}) 
	
	
	$reduce_cost.blur(function(){
	...
	var save = function(){
			var res = {};
				...
				res['reduce_cost'] = $reduce_cost.val(); 
				res['title'] = $title.val();
				res['sub_title'] = $sub_title.val();
				res['least_cost'] = $least_cost.val(); 
...
var test_data = {
		...
		end_timestamp: "2014-05-12",
		type: 1 ,
		description: "濞达綀娉曢弫銈嗐亜閼姐倗鍙�,
		discount: "1",
		get_limit: "2",
		notice: "闁瑰灝绉崇紞鏃堝箵閹邦喓浠�,
		quantity: "1",
		service_phone: "12321323",
		sub_title: "闁告搩鍨遍悥锝嗭紣閿燂拷
		title: "闁硅埖蓱婢ф悂宕氶崨濠勫灱濡府鎷�	
}
*/